...
if (root) {
    ...
    function renderGarage() {
        const cards = state.vehicles.length
            ? state.vehicles.map(v => `<article class="rounded-xl border border-pm-border bg-pm-surface p-4"><div class="flex items-start justify-between gap-3"><div><h3 class="font-bold text-pm-text">${esc(v.name)}</h3><p class="mt-1 text-sm text-pm-text-secondary">${esc([v.manufacturer, v.model, v.year].filter(Boolean).join(' · '))}</p><span class="mt-3 inline-block rounded-full bg-pm-subtle px-2.5 py-1 text-xs text-pm-muted">${esc(v.type)}</span></div>${rowActions('vehicles', v.id)}</div></article>`).join('')
    ...
    function renderConfigurations() {
        ...
        const checks = state.components.length
            ? state.components.map(c=>`<label class="flex items-center gap-2 rounded-lg border border-pm-border bg-pm-subtle px-3 py-2 text-sm text-pm-text-secondary"><input type="checkbox" name="componentIds" value="${c.id}" class="rounded border-pm-border-strong text-pm-accent">${esc(c.name)}</label>`).join('')
    ...
    function renderCircuits() {
        const cards = state.circuits.length
            ? state.circuits.map(c=>`<article class="rounded-xl border border-pm-border bg-pm-surface p-4"><div class="flex justify-between gap-3"><div><h3 class="font-bold text-pm-text">${esc(c.name)}</h3><p class="mt-1 text-sm text-pm-text-secondary">${esc(c.layout)}</p><p class="mt-3 font-mono text-sm text-pm-text">${Number(c.lengthMeters).toLocaleString(it?'it-IT':'en-GB')} m</p></div>${rowActions('circuits',c.id)}</div></article>`).join('')
    ...
    function renderMaintenance() {
        ...
        const rows = state.maintenance.length
            ? state.maintenance.slice().reverse().map(m=>`<article class="rounded-xl border border-pm-border bg-pm-surface p-4"><div class="flex justify-between gap-3"><div><p class="text-xs text-pm-muted">${dateFmt(m.date)}</p><h3 class="mt-1 font-bold text-pm-text">${esc(nameOf(state.components,m.componentId))}</h3><p class="mt-1 text-sm text-pm-text-secondary">${esc(m.action)}</p>${m.notes?`<p class="mt-2 text-sm text-pm-muted">${esc(m.notes)}</p>`:''}${m.resetUsage?`<span class="mt-3 inline-block rounded-full bg-pm-success-subtle px-2.5 py-1 text-xs font-semibold text-pm-success">${it?'Contatore azzerato':'Counter reset'}</span>`:''}</div>${rowActions('maintenance',m.id)}</div></article>`).join('')
            : empty(it?'Nessun intervento registrato.':'No maintenance records yet.');

        return `${heading('MAINTENANCE',it?'Storico manutenzione':'Maintenance history',it?'Registra controlli, revisioni e sostituzioni e, quando serve, azzera il contatore del componente dopo l’intervento.':'Record inspections, rebuilds and replacements and reset the component counter after service when needed.')}${panel(`<form data-form="maintenance" class="grid gap-4 md:grid-cols-2">${select('componentId',it?'Componente':'Component',options,'required')}${input('date',it?'Data':'Date','date','required')}${input('action',it?'Intervento':'Action','text','required placeholder="Rebuild"')}${input('notes',it?'Note':'Notes')}<label class="flex items-center gap-2 text-sm text-pm-text-secondary"><input type="checkbox" name="resetUsage" value="1" class="rounded border-pm-border-strong text-pm-accent">${it?'Azzera il contatore utilizzo dopo l’intervento':'Reset usage counter after this service'}</label><div class="md:col-span-2">${primary(it?'Registra intervento':'Add maintenance')}</div></form>`)}<div class="grid gap-3 md:grid-cols-2">${rows}</div>`;
    }
    ...
    document.getElementById('demo-seed')?.addEventListener('click', () => {
        ...
        state = {
            version:2,
            vehicles:[{id:vehicleId,name:'Kart 125',type:'Kart',manufacturer:'Tony Kart',model:'Racer 401',year:'2026'}],
            ...
            circuits:[{id:circuitId,name:'Kart Track',layout:'Full',lengthMeters:1250}],
