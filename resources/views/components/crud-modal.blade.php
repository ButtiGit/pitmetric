...
<dialog id="{{ $id }}" data-pm-crud-dialog aria-labelledby="{{ $id }}-title" @if ($description) aria-describedby="{{ $id }}-description" @endif class="w-[min(94vw,72rem)] {{ $size }} rounded-2xl border border-pm-border bg-pm-panel p-0 text-pm-text shadow-2xl backdrop:bg-black/70">
    <div class="max-h-[88vh] overflow-y-auto p-5 sm:p-6" data-pm-dialog-scroll>
        <div class="hidden" aria-hidden="true" data-pm-dialog-handle></div>

        <div class="mb-5 flex items-start justify-between gap-4" data-pm-dialog-header>
            <div class="min-w-0">
            ...
            <button type="button" class="grid size-10 shrink-0 place-items-center rounded-lg border border-pm-border bg-pm-subtle text-lg text-pm-muted transition hover:text-pm-text" onclick="document.getElementById('{{ $id }}').close()" aria-label="{{ __('Close') }}">×</button>
        </div>

        <div data-pm-dialog-body>
        ...
        <div class="mt-4 border-t border-pm-border pt-3" data-pm-dialog-footer><button type="button" class="pm-ghost-button" onclick="document.getElementById('{{ $id }}').close()">{{ __('workflow.cancel') }}</button></div>
    </div>
</dialog>
@endif
