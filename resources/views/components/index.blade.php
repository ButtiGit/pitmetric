<x-layouts::app :title="__('Components')">
    ...
    <div class="pitmetric-app min-h-full w-full bg-pm-page px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <div class="mx-auto w-full max-w-[1360px] space-y-5">
            <div><p class="text-[11px] font-bold uppercase tracking-[0.14em] text-pm-accent">COMPONENTS</p><x-pitmetric.page-header :title="$it ? 'Componenti reali' : 'Real components'" :description="$it ? 'Ogni componente è salvato nel database e il suo utilizzo deriva dal ledger storico.' : 'Every component is stored in the database and its usage comes from the historical ledger.'" /></div>
            ...
            <section class="pm-panel p-5 sm:p-6">
                <h2 class="text-lg font-black text-pm-text">{{ $it ? 'Aggiungi componente' : 'Add component' }}</h2>
                <form method="POST" action="{{ route('components.store') }}" class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    @csrf
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Nome' : 'Name' }}</span><input class="pm-input" name="name" required maxlength="120" placeholder="Engine #01"></label>
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Tipo' : 'Type' }}</span><input class="pm-input" name="type_name" required maxlength="100" placeholder="Engine"></label>
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Metrica principale' : 'Primary metric' }}</span><select class="pm-input" name="metric_key" required>@foreach ($metrics as $metric)<option value="{{ $metric->key }}">{{ $metric->name }} · {{ $metric->display_unit }}</option>@endforeach</select></label>
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Seriale / codice' : 'Serial / code' }}</span><input class="pm-input" name="serial_number" maxlength="120"></label>
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Marca' : 'Manufacturer' }}</span><input class="pm-input" name="manufacturer" maxlength="100"></label>
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Modello' : 'Model' }}</span><input class="pm-input" name="model" maxlength="100"></label>
                    <label class="grid gap-2 md:col-span-2"><span class="pm-label">{{ $it ? 'Note' : 'Notes' }}</span><input class="pm-input" name="notes" maxlength="2000"></label>
