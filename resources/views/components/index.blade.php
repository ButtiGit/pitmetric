            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="flex flex-wrap gap-2">
                    <x-crud-modal id="create-component" :title="$it ? 'Aggiungi componente' : 'Add component'" :description="$it ? 'Il costo di acquisto viene registrato subito anche nei Costi.' : 'Purchase cost is recorded in Expenses immediately.'" :trigger="$it ? '+ Componente' : '+ Component'">
                        @can('team-write')<form method="POST" action="{{ route('components.store') }}" class="grid gap-4 md:grid-cols-2">
                            @csrf
                    ...
                    <x-crud-modal id="install-component" :title="$it ? 'Monta componente' : 'Install component'" :description="$it ? 'Registri posizione, data e costo dell’operazione in un solo passaggio.' : 'Record position, date and operation cost in one step.'" :trigger="$it ? 'Monta componente' : 'Install component'" trigger-class="pm-ghost-button">
                        @can('team-write')<form method="POST" action="{{ route('component-installations.store') }}" class="grid gap-4 md:grid-cols-2">
                            @csrf
            ...
            @else
                <section class="pm-panel overflow-hidden">
                    <div class="overflow-x-auto">
                        <table data-pm-responsive-table class="w-full min-w-[1180px] text-left text-sm">
                            <thead class="border-b border-pm-border bg-pm-subtle text-[11px] uppercase tracking-[0.1em] text-pm-muted"><tr><th class="px-4 py-3">{{ $it ? 'Componente' : 'Component' }}</th><th class="px-4 py-3">{{ $it ? 'Tipo' : 'Type' }}</th><th class="px-4 py-3">{{ $it ? 'Montato su' : 'Installed on' }}</th><th class="px-4 py-3">{{ $it ? 'Metrica' : 'Metric' }}</th><th class="px-4 py-3">{{ $it ? 'Dall’ultimo service' : 'Since service' }}</th><th class="px-4 py-3">{{ $it ? 'Utilizzo totale' : 'Lifetime' }}</th><th class="px-4 py-3">{{ $it ? 'Acquisto' : 'Purchase' }}</th><th class="px-4 py-3"></th></tr></thead>
                            <tbody class="divide-y divide-pm-border">
                                @foreach ($components as $part)
                                    ...
                                    <tr id="component-{{ $part->id }}" class="scroll-mt-28 transition-colors target:bg-pm-accent/5">
                                        ...
                                        <td class="px-4 py-4">
                                            @if ($installation)
                                                <a href="{{ route('garage.index') }}#vehicle-{{ $installation->vehicle_id }}" class="font-bold text-pm-accent">{{ $installation->vehicle->name }}</a>
                                                ...
                                                <div class="mt-2">
                                                    <x-crud-modal id="remove-installation-{{ $installation->id }}" :title="$it ? 'Rimuovi componente' : 'Remove component'" :description="$part->name.' · '.$installation->vehicle->name" :trigger="$it ? 'Rimuovi dal mezzo' : 'Remove from vehicle'" trigger-class="text-xs font-bold text-pm-accent hover:underline">
                                                        @can('team-write')<form method="POST" action="{{ route('component-installations.remove', $installation) }}" class="grid gap-4 sm:grid-cols-2">
                                                            @csrf @method('PATCH')
                                        ...
                                        <td class="px-4 py-4 text-right"><x-crud-modal id="edit-component-{{ $part->id }}" :title="$it ? 'Modifica componente' : 'Edit component'" :trigger="$it ? 'Modifica componente' : 'Edit component'" trigger-class="pm-ghost-button">
 @can('team-write')<form method="POST" action="{{ route('components.update', $part) }}" class="grid gap-4 sm:grid-cols-2">@csrf @method('PUT')
 <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Nome' : 'Name' }}</span><input class="pm-input" name="name" type="text" value="{{ $part->name }}" required maxlength="120"></label>
<label class="grid gap-2"><span class="pm-label">{{ $it ? 'Seriale / codice' : 'Serial / code' }}</span><input class="pm-input" name="serial_number" type="text" value="{{ $part->serial_number }}" maxlength="120"></label>
<label class="grid gap-2"><span class="pm-label">{{ $it ? 'Marca' : 'Manufacturer' }}</span><input class="pm-input" name="manufacturer" type="text" value="{{ $part->manufacturer }}" maxlength="100"></label>
<label class="grid gap-2"><span class="pm-label">{{ $it ? 'Modello' : 'Model' }}</span><input class="pm-input" name="model" type="text" value="{{ $part->model }}" maxlength="100"></label>
<label class="grid gap-2"><span class="pm-label">{{ $it ? 'Note' : 'Notes' }}</span><textarea class="pm-input min-h-20" name="notes" maxlength="2000">{{ $part->notes }}</textarea></label>
