        ...
        <div class="mx-auto w-full max-w-[1360px] space-y-4 sm:space-y-5">
            ...
            <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                @foreach ([
                    ...
                    <a href="{{ route($routeName) }}" class="group rounded-xl border border-pm-border bg-pm-surface p-4 transition hover:border-pm-border-strong hover:bg-pm-hover sm:p-5"><div class="flex items-center justify-between gap-3"><h2 class="min-w-0 font-bold text-pm-text">{{ $title }}</h2><span class="relative block size-3 shrink-0 text-pm-muted transition group-hover:translate-x-0.5 group-hover:text-pm-accent" aria-hidden="true"><span class="absolute right-0 top-1/2 h-px w-3 -translate-y-1/2 bg-current"></span><span class="absolute right-0 top-1/2 size-1.5 -translate-y-1/2 rotate-45 border-r border-t border-current"></span></span></div><p class="mt-2 text-sm text-pm-text-secondary">{{ $subtitle }}</p></a>
                @endforeach
            </section>

            <section class="pm-panel p-4 sm:p-6"><div class="flex items-start gap-3"><div class="grid size-9 shrink-0 place-items-center rounded-lg border border-pm-border bg-pm-success-subtle text-[10px] font-black tracking-[0.08em] text-pm-success">OK</div><div class="min-w-0"><h2 class="font-bold text-pm-text">{{ $it ? 'Pronto per la prossima sessione' : 'Ready for the next session' }}</h2><p class="mt-1 text-sm leading-6 text-pm-text-secondary">{{ $it ? 'Usa le sezioni del gestionale per preparare il mezzo, registrare il lavoro svolto e mantenere sotto controllo utilizzo e costi.' : 'Use the manager sections to prepare the vehicle, record completed work and keep usage and costs under control.' }}</p></div></div></section>
        </div>
    </div>
</x-layouts::app>
