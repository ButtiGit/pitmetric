        ...
        <div class="mx-auto w-full max-w-[1360px] space-y-5">
            ...
            @if ($attentionSchedules->isNotEmpty())
                <section class="pm-panel border-pm-warning/25 p-5 sm:p-6">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-[11px] font-bold uppercase tracking-[0.12em] text-pm-warning">{{ $it ? 'Prima di girare' : 'Before running' }}</p><h2 class="mt-2 text-lg font-black text-pm-text">{{ $it ? 'Controlla gli interventi in attenzione' : 'Check maintenance items needing attention' }}</h2></div><a href="{{ route('demo.maintenance') }}" class="pm-ghost-button">{{ $it ? 'Apri manutenzione' : 'Open maintenance' }}</a></div>
                    <div class="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">@foreach ($attentionSchedules->take(6) as $schedule)@php($state = $maintenanceStates[$schedule->id])<div class="rounded-xl border border-pm-border bg-pm-subtle p-3"><div class="flex items-start justify-between gap-3"><div><p class="font-bold text-pm-text">{{ $schedule->tracker->component->name }}</p><p class="mt-1 text-xs text-pm-muted">{{ $schedule->name }}</p></div><x-pitmetric.status-badge :label="$state['status']" :variant="$state['status'] === 'overdue' ? 'danger' : 'warning'" /></div></div>@endforeach</div>
                </section>
            @endif

            <section class="pm-panel p-5 sm:p-6">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 class="text-lg font-black text-pm-text">{{ $it ? 'Registra sessione' : 'Record session' }}</h2><p class="mt-1 text-sm text-pm-text-secondary">{{ $sessions->isNotEmpty() ? ($it ? 'Configurazione, circuito e tipo sono precompilati dall’ultima sessione per ridurre i click in pista.' : 'Configuration, circuit and type are prefilled from the last session to reduce trackside clicks.') : ($it ? 'La prima sessione creerà la base dello storico operativo.' : 'The first session will establish the operational history.') }}</p></div></div>
                <form method="POST" action="{{ route('sessions.store') }}" class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Tipo sessione' : 'Session type' }}</span><select class="pm-input" name="session_type" required>@foreach (['practice' => 'Practice', 'qualifying' => 'Qualifying', 'race' => 'Race', 'test' => 'Test'] as $value => $label)<option value="{{ $value }}" @selected($selectedType === $value)>{{ $label }}</option>@endforeach</select></label>
                    ...
                    <label class="grid gap-2 md:col-span-2 xl:col-span-4"><span class="pm-label">{{ $it ? 'Note' : 'Notes' }}</span><input class="pm-input" name="notes" maxlength="4000" value="{{ old('notes') }}"></label>
                    <div class="md:col-span-2 xl:col-span-4"><button class="pm-race-button" type="submit" @disabled($versions->isEmpty())>{{ $it ? 'Registra, aggiorna utilizzo e controlla alert' : 'Record, update usage and check alerts' }}</button></div>
                </form>
            </section>
