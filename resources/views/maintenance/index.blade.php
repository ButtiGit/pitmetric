<x-layouts::app :title="__('Maintenance')">
    ...
    <div class="pitmetric-app min-h-full w-full bg-pm-page px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <div class="mx-auto w-full max-w-[1360px] space-y-5">
            <div><p class="text-[11px] font-bold uppercase tracking-[0.14em] text-pm-accent">MAINTENANCE</p><x-pitmetric.page-header :title="$it ? 'Manutenzione basata sull’utilizzo' : 'Usage-based maintenance'" :description="$it ? 'Gli alert derivano dall’utilizzo reale delle sessioni. Quando completi un intervento, l’intervallo riparte senza cancellare il lifetime e il costo entra automaticamente nelle spese.' : 'Alerts come from real session usage. Completing service restarts the interval without erasing lifetime usage, and its cost is added to expenses automatically.'" /></div>
            ...
            <section class="grid gap-3 sm:grid-cols-3">
                <article class="pm-stat-card"><p class="text-[11px] font-semibold uppercase tracking-[0.12em] text-pm-muted">Overdue</p><p class="mt-3 text-3xl font-black text-pm-danger">{{ $summary['overdue'] }}</p></article>
                <article class="pm-stat-card"><p class="text-[11px] font-semibold uppercase tracking-[0.12em] text-pm-muted">Due soon</p><p class="mt-3 text-3xl font-black text-pm-warning">{{ $summary['due_soon'] }}</p></article>
                <article class="pm-stat-card"><p class="text-[11px] font-semibold uppercase tracking-[0.12em] text-pm-muted">{{ $it ? 'Piani OK' : 'Schedules OK' }}</p><p class="mt-3 text-3xl font-black text-pm-success">{{ $summary['ok'] }}</p></article>
            </section>
            ...
            <section class="pm-panel p-5 sm:p-6">
                ...
                <form method="POST" action="{{ route('maintenance.store') }}" class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">@csrf
                    ...
                    <label class="grid gap-2"><span class="pm-label">{{ $it ? 'Intervallo' : 'Interval' }}</span><input class="pm-input" name="interval_display" type="number" required min="0.01" step="0.01" value="{{ old('interval_display') }}"><span class="text-xs text-pm-muted">{{ $it ? 'Usa l’unità mostrata nella metrica.' : 'Use the unit shown by the metric.' }}</span></label>
                    ...
                    <label class="grid gap-2 md:col-span-2 xl:col-span-3"><span class="pm-label">{{ $it ? 'Note' : 'Notes' }}</span><input class="pm-input" name="notes" maxlength="2000" value="{{ old('notes') }}"></label>
            ...
            <section class="grid gap-4 lg:grid-cols-2">
                @forelse ($schedules as $schedule)
                    ...
                    <article class="pm-panel p-5 sm:p-6 {{ $state['status'] === 'overdue' ? 'border-pm-danger/30' : ($state['status'] === 'due_soon' ? 'border-pm-warning/30' : '') }}">
                        ...
                        <div class="mt-4 grid grid-cols-3 gap-3"><div class="rounded-xl border border-pm-border bg-pm-subtle p-3"><p class="text-[11px] uppercase tracking-[0.1em] text-pm-muted">{{ $it ? 'Da service' : 'Since service' }}</p><p class="mt-2 font-mono font-bold text-pm-text">{{ $formatUsage($state['used'], $schedule->tracker->metric) }}</p></div><div class="rounded-xl border border-pm-border bg-pm-subtle p-3"><p class="text-[11px] uppercase tracking-[0.1em] text-pm-muted">{{ $it ? 'Limite' : 'Limit' }}</p><p class="mt-2 font-mono font-bold text-pm-text">{{ $formatUsage($schedule->interval_value, $schedule->tracker->metric) }}</p></div><div class="rounded-xl border border-pm-border bg-pm-subtle p-3"><p class="text-[11px] uppercase tracking-[0.1em] text-pm-muted">{{ $state['remaining'] < 0 ? ($it ? 'Oltre limite' : 'Over limit') : ($it ? 'Residuo' : 'Remaining') }}</p><p class="mt-2 font-mono font-bold {{ $state['remaining'] < 0 ? 'text-pm-danger' : 'text-pm-text' }}">{{ $formatUsage(abs($state['remaining']), $schedule->tracker->metric) }}</p></div></div>
                        <p class="mt-3 text-xs text-pm-muted">Lifetime: {{ $formatUsage($state['lifetime'], $schedule->tracker->metric) }}</p>
                        <details class="mt-5 border-t border-pm-border pt-4"><summary class="cursor-pointer text-sm font-bold text-pm-text hover:text-pm-accent">{{ $it ? 'Registra intervento completato' : 'Record completed maintenance' }}</summary><form method="POST" action="{{ route('maintenance.complete', $schedule) }}" class="mt-4 grid gap-3 sm:grid-cols-2">@csrf<label class="grid gap-2"><span class="pm-label">{{ $it ? 'Data e ora' : 'Date and time' }}</span><input class="pm-input" name="performed_at" type="datetime-local" required value="{{ now()->format('Y-m-d\TH:i') }}"></label><label class="grid gap-2"><span class="pm-label">{{ $it ? 'Costo (€)' : 'Cost (€)' }}</span><input class="pm-input" name="cost" type="number" min="0" step="0.01"><span class="text-xs text-pm-muted">{{ $it ? 'Il costo verrà aggiunto alle spese.' : 'The cost will be added to expenses.' }}</span></label><label class="grid gap-2 sm:col-span-2"><span class="pm-label">{{ $it ? 'Intervento' : 'Work performed' }}</span><input class="pm-input" name="description" required maxlength="180" value="{{ $schedule->name }}"></label><label class="grid gap-2 sm:col-span-2"><span class="pm-label">{{ $it ? 'Note' : 'Notes' }}</span><input class="pm-input" name="notes" maxlength="2000"></label><div class="sm:col-span-2"><button class="pm-ghost-button" type="submit">{{ $it ? 'Completa, resetta e registra costo' : 'Complete, reset and record cost' }}</button></div></form></details>
                    </article>
                @empty
            ...
            @if ($records->isNotEmpty())<section class="pm-panel p-5 sm:p-6"><h2 class="text-lg font-black text-pm-text">{{ $it ? 'Storico recente' : 'Recent history' }}</h2><div class="mt-4 space-y-3">@foreach ($records as $record)<div class="flex flex-col gap-1 rounded-xl border border-pm-border bg-pm-subtle p-3 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-bold text-pm-text">{{ $record->component->name }} · {{ $record->description }}</p><p class="mt-1 text-xs text-pm-muted">{{ $record->performed_at->format('d/m/Y H:i') }}</p></div>@if ($record->cost_cents !== null)<div class="text-right"><p class="font-mono font-bold text-pm-text">€ {{ number_format($record->cost_cents / 100, 2, ',', '.') }}</p><p class="mt-1 text-[10px] uppercase tracking-[0.1em] text-pm-muted">{{ $it ? 'collegato alle spese' : 'linked to expenses' }}</p></div>@endif</div>@endforeach</div></section>@endif
        </div>
    </div>
</x-layouts::app>
