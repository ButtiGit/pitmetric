    ...
    <div class="pitmetric-app min-h-full w-full bg-pm-page px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <div class="mx-auto w-full max-w-[1360px] space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div><p class="text-[11px] font-bold uppercase tracking-[0.14em] text-pm-accent">MAINTENANCE</p><x-pitmetric.page-header :title="$it ? 'Manutenzione basata sull’utilizzo' : 'Usage-based maintenance'" :description="$it ? 'Gli alert derivano dall’utilizzo reale delle sessioni. Completando un intervento, il costo entra automaticamente nelle spese.' : 'Alerts come from real session usage. Completing service automatically sends its cost to Expenses.'" /></div>
                <x-crud-modal id="create-maintenance-schedule" :title="$it ? 'Nuovo piano manutenzione' : 'New maintenance schedule'" :description="$it ? 'Definisci l’intervallo solo quando serve, senza tenere il form sempre aperto.' : 'Define the interval only when needed, without keeping the form always visible.'" :trigger="$it ? '+ Piano manutenzione' : '+ Maintenance schedule'">
                    <form method="POST" action="{{ route('maintenance.store') }}" class="grid gap-4 md:grid-cols-2">@csrf
                ...
                @forelse ($schedules as $schedule)
                    ...
                    <article class="pm-panel p-5 sm:p-6 {{ $state['status'] === 'overdue' ? 'border-pm-danger/30' : ($state['status'] === 'due_soon' ? 'border-pm-warning/30' : '') }}">
                        ...
                        <div class="mt-5 border-t border-pm-border pt-4">
                            <x-crud-modal id="complete-maintenance-{{ $schedule->id }}" :title="$it ? 'Registra intervento completato' : 'Record completed maintenance'" :description="$schedule->tracker->component->name.' · '.$schedule->name" :trigger="$it ? 'Completa intervento' : 'Complete maintenance'" trigger-class="pm-ghost-button">
                                <form method="POST" action="{{ route('maintenance.complete', $schedule) }}" class="grid gap-4 sm:grid-cols-2">@csrf
