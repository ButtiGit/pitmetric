<x-layouts.public>
    <section class="mx-auto max-w-7xl px-5 pb-20 pt-20 lg:px-8 lg:pt-28">
        <div class="max-w-4xl">
            ...
            <h1 class="mt-7 text-5xl font-black tracking-[-0.04em] text-white sm:text-6xl lg:text-7xl">Know every lap.<br><span class="text-[#FF5A36]">Track every component.</span></h1>
            ...
            <div class="mt-9 flex flex-wrap gap-3">
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-xl bg-[#FF5A36] px-5 py-3 font-semibold text-[#160b08]">Apri dashboard</a>
                @else
                    <a href="{{ route('register') }}" class="rounded-xl bg-[#FF5A36] px-5 py-3 font-semibold text-[#160b08]">Scopri PitMetric</a>
                    <a href="{{ route('login') }}" class="rounded-xl border border-white/15 px-5 py-3 font-semibold text-white">Accedi</a>
                @endauth
            </div>
        </div>
        ...
        <div class="mt-20 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            @foreach ([
                ...
                <article class="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                    <div class="mb-8 h-1 w-10 rounded-full bg-[#FF5A36]"></div>
    ...
    <section class="border-y border-white/10 bg-white/[0.025]">
        <div class="mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.18em] text-[#ff8a70]">Il principio</p>
    ...
    <section class="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div><p class="text-sm font-semibold uppercase tracking-[0.18em] text-[#ff8a70]">Roadmap</p><h2 class="mt-3 text-3xl font-bold text-white">Costruito per il lavoro reale in pista.</h2></div>
        ...
        <div class="mt-8 grid gap-4 md:grid-cols-3">
            @foreach ([
                ...
                <article class="rounded-2xl border border-white/10 p-6"><span class="font-mono text-xs text-[#ff8a70]">{{ $number }}</span><h3 class="mt-5 font-bold text-white">{{ $heading }}</h3><p class="mt-3 text-sm leading-6 text-zinc-400">{{ $copy }}</p></article>
            @endforeach
        </div>
    </section>
</x-layouts.public>
