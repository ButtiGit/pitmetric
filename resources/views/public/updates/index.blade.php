        ...
        <div class="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            @forelse ($updates as $update)
                <article class="flex min-h-64 flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                    ...
                    <h2 class="mt-4 text-xl font-bold text-white"><a href="{{ route('updates.show', $update) }}" class="hover:text-[#ff8a70]">{{ $update->title }}</a></h2>
                    <p class="mt-4 text-sm leading-6 text-zinc-400">{{ $update->excerpt }}</p>
                    <a href="{{ route('updates.show', $update) }}" class="mt-auto pt-7 text-sm font-semibold text-[#ff8a70]">Leggi update →</a>
                </article>
            @empty
