<x-layouts::app :title="__('pitmetric.studio.title')">
    <div class="pitmetric-app min-h-full w-full bg-pm-page px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <div class="mx-auto w-full max-w-[1260px] space-y-5">
            @if (session('status'))
                ...
                <div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-pm-success">
                            ...
                            {{ __('pitmetric.studio.ready_to_publish') }}
                        </div>
                        <h1 class="mt-4 text-3xl font-black tracking-[-0.035em] text-pm-text sm:text-4xl">{{ __('pitmetric.studio.title') }}</h1>
            ...
            <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                @forelse ($updates as $update)
                    <article class="pm-race-card group flex min-h-72 flex-col overflow-hidden">
                        @if ($update->mediaSource() && $update->media_type === 'image')
                            <div class="h-36 overflow-hidden border-b border-pm-border"><img src="{{ $update->mediaSource() }}" alt="" class="h-full w-full object-cover transition duration-500 group-hover:scale-[1.02]"></div>
                        @elseif ($update->media_type === 'video')
                            <div class="grid h-36 place-items-center border-b border-pm-border bg-pm-subtle"><div class="pm-play-mark scale-50"><span></span></div></div>
                        @endif
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span class="text-xs text-pm-muted">{{ $update->updated_at?->format('d/m/Y') }}</span>
                            </div>
                ...
                @empty
                    <div class="pm-panel col-span-full p-12 text-center sm:p-16">
                        <div class="mx-auto grid size-12 place-items-center rounded-xl border border-pm-border bg-pm-subtle text-xl text-pm-accent">＋</div>
