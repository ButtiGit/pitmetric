                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <x-crud-modal
                            ...
                            :description="$it ? 'I campi restano nascosti finché non servono. Se inserisci il costo, viene creato automaticamente nei Costi.' : 'Fields stay out of the way until needed. Entering a cost automatically creates the matching expense.'"
                        ...
                        >
                            <form method="POST" action="{{ route('garage.store') }}" class="grid gap-4 md:grid-cols-2">
                                ...
                                <label class="grid gap-2"><span class="pm-label">{{ __('garage.fields.category') }}</span><select class="pm-input" name="category" required>@foreach ($categoryLabels as $value => $label)<option value="{{ $value }}" @selected(old('category', 'kart') === $value)>{{ $label }}</option>@endforeach</select></label>
                    ...
                    @foreach ($vehicles as $vehicle)
                        <article class="pm-panel p-5 sm:p-6">
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2"><h3 class="truncate text-lg font-black text-pm-text">{{ $vehicle->name }}</h3><span class="rounded-full px-2.5 py-1 text-[11px] font-bold {{ $vehicle->status === 'active' ? 'bg-pm-success-subtle text-pm-success' : 'bg-pm-subtle text-pm-muted' }}">{{ $statusLabels[$vehicle->status] ?? $vehicle->status }}</span></div>
                                    <p class="mt-1 text-sm text-pm-text-secondary">{{ $categoryLabels[$vehicle->category] ?? $vehicle->category }}@if ($vehicle->manufacturer || $vehicle->model) · {{ collect([$vehicle->manufacturer, $vehicle->model])->filter()->join(' ') }}@endif @if ($vehicle->year) · {{ $vehicle->year }}@endif</p>
                                    @if ($vehicle->identifier)<p class="mt-2 font-mono text-xs text-pm-muted">{{ $vehicle->identifier }}</p>@endif
                                    @if ($vehicle->notes)<p class="mt-4 text-sm leading-6 text-pm-text-secondary">{{ $vehicle->notes }}</p>@endif
                                </div>
                            ...
                            <div class="mt-5 flex flex-wrap gap-2 border-t border-pm-border pt-4">
                                <x-crud-modal id="edit-vehicle-{{ $vehicle->id }}" :title="$it ? 'Modifica mezzo' : 'Edit vehicle'" :trigger="__('garage.edit.toggle')" trigger-class="pm-ghost-button">
                                    <form method="POST" action="{{ route('garage.update', $vehicle) }}" class="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <label class="grid gap-2"><span class="pm-label">{{ __('garage.fields.category') }}</span><select class="pm-input" name="category" required>@foreach ($categoryLabels as $value => $label)<option value="{{ $value }}" @selected($vehicle->category === $value)>{{ $label }}</option>@endforeach</select></label>
